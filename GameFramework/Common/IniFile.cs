...
namespace GameFramework
{
    ...
    public class IniFile
    {
        ...
        #region Sync Methods:added by figo
        ...
        public static void SyncIniUint(IniFile conf, string section, string key, ref uint uIntCmd)
        {
            int nLoadInteger = conf.ReadInteger(section, key, -1);
            if (nLoadInteger < 0)
            {
                conf.WriteInteger(section, key, uIntCmd);
            }
            else
            {
                uIntCmd = conf.ReadInteger(section, key, uIntCmd);
            }
        }
        ...
        public static void SyncIniDouble(IniFile conf, string section, string key, ref double doubleCmd)
        {
            double nLoadFloat = conf.ReadFloat(section, key, 0);
            if (nLoadFloat == 0)
            {
                conf.WriteFloat(section, key, doubleCmd);
            }
            else
            {
                doubleCmd = nLoadFloat;
            }
        }
        public static void SyncIniSbyte(IniFile conf, string section, string key, ref sbyte sByteCmd)
        {
            int nLoadInteger = conf.ReadInteger(section, key, -1);
            if (nLoadInteger < 0)
            {
                conf.WriteInteger(section, key, sByteCmd);
            }
            else
            {
                sByteCmd = (sbyte)conf.ReadInteger(section, key, sByteCmd);
            }
        }
        #endregion
    }
