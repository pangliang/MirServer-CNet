...
namespace NetFramework.AsyncSocketServer
{
    ...
    public class IServerSocket
    {
        ...
        // 添加功能
        private int m_numConnections;           // 设计同时处理的连接最大数

        private int m_BufferSize;               // 用于每一个Socket I/O操作使用的缓冲区大小
        private BufferManager m_bufferManager;  // 为所有Socket操作准备的一个大的可重用的缓冲区集合
        private const int opsToPreAlloc = 2;    // 需要分配空间的操作数目:为 读、写、接收等 操作预分配空间(接受操作可以不分配)
        private Socket listenSocket;            // 用来侦听到达的连接请求的Socket
        ...
        internal void Start(IPEndPoint localEndPoint)// 启动
        {
            try
            {
                // 创建一个侦听到达连接的Socket
        ...
        private void ProcessSend(SocketAsyncEventArgs e)
        {
            ...
            {
                Debug.WriteLine(string.Format("发送 字节数:{0}", e.BytesTransferred));
        ...
        private void CloseClientSocket(AsyncUserToken token)
        {
            ...
            finally
            {
                ...
                Debug.WriteLine(string.Format("一个客户端被从服务器断开. 有 {0} 个客户端连接到服务器", m_numConnectedSockets));
                lock (m_readPool)
                {
                    // 释放以使它们可以被其他客户端重新利用
                    m_readPool.Push(token.ReadEventArgs);
