...
namespace M2Server
{
    public class BaseCommond : GameBase
    {
        ...
        public virtual string Handle(string parameters, TPlayObject PlayObject = null)
        {
            ...
            {
                return GameMsgDef.g_sGameCommandPermissionTooLow;//权限不足
            }
            ...
            int MethodsParamsCount = this._commands[target].GetParameters().Length;//查看命令目标所需要的参数个数
            if (MethodsParamsCount > 1) //默认参数为当前对象，即：PlayObject
            {
                result = (string)this._commands[target].Invoke(this, new object[] { PlayObject, @params });
            }
            return result;
        }
